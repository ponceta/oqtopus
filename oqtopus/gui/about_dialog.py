...
def _versions_json(libs_dir: str) -> dict[str, str]:
    """Load ``versions.json`` from *libs_dir* if it exists."""
    versions_file = os.path.join(libs_dir, "versions.json")
...
def get_library_version(name: str) -> dict[str, str]:
    ...
    # Fallback: read from versions.json generated by download-libs.sh
    if version is None:
        version = _versions_json(libs_dir).get(name)
